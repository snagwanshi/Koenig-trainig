/**
 * Admin API client — the WRITE half of the two-token pattern.
 *
 * An Admin access token obtained by client-credentials exchange expires (24
 * hours), so this module caches one and re-exchanges before it goes stale.
 * The client id and secret are the durable credentials; the access token is
 * disposable.
 */
...
function requireEnv(name, ...aliases) {
  ...
  throw new Error(
    `Missing ${name}. Add it to lab5-shopify-agent-complete/.env.`,
  );
}
